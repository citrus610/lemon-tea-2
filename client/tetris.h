...
static void create_json() 
{
    ...
    // Setting data
    json js;
    LemonTea::Config config;
    config.heuristic[0] = LemonTea::DEFAULT_HEURISTIC();
    config.heuristic[1] = LemonTea::DEFAULT_HEURISTIC();
    to_json(js, config);
...
static void set_from_json(json& js, LemonTea::Config& config) 
{
    from_json(js, config);
};
